...
import com.utils.MySessionContext;
import com.utils.RandomNum;

public class Upload extends HttpServlet {
    public void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
    	String description = request.getParameter("photo");
        String savePath = this.getServletConfig().getServletContext().getRealPath("");
        HttpSession session = request.getSession(true);
////        HttpSession session = request.getSession(); 
//        String jsessionid = request.getParameter("jsessionid");
//        Object sessionobj = request.getAttribute("sessionobj");
//        
//        HttpSession session2 =   MySessionContext.getSession(jsessionid);
//        String id = session.getId();
//        HttpSession session3 =   MySessionContext.getSession(jsessionid);
        if(session!=null){
